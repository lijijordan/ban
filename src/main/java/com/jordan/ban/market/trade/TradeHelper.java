...
//        System.out.println(marketDepth.toString());
        double minVolume = 1;
        double tradeBuy = Math.max(marketDepth.getD1ask(), marketDepth.getD1bid()) * minVolume;
        double tradeSell = Math.min(marketDepth.getD2ask(), marketDepth.getD2bid()) * minVolume;
...
//        System.out.println("Market 1 buy , Market 2 sell. diff=" + (float) tradeDiff);
        ...
        return new MockTradeResult(tradeDiff, tradeDiff / (tradeBuy + tradeSell), TradeDirect.A2B,
                Math.min(marketDepth.getVolume(tradeBuy), marketDepth.getVolume(tradeSell)));
    }

    public static MockTradeResult eatB2A(MarketDepth marketDepth) {
        double minVolume = 1;
        double tradeBuy = Math.max(marketDepth.getD2ask(), marketDepth.getD2bid()) * minVolume;
        double tradeSell = Math.min(marketDepth.getD1ask(), marketDepth.getD1bid()) * minVolume;
        double tradeDiff = tradeSell - tradeBuy;
...
//        System.out.println("Market 1 buy , Market 2 sell. diff=" + (float) tradeDiff);
        // TODO:减去手续费
        tradeDiff = tradeDiff - Math.abs(Math.max(tradeBuy, tradeSell) * TRADE_FEES * 2);
        return new MockTradeResult(tradeDiff, tradeDiff / (tradeBuy + tradeSell),
